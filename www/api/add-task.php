<?php
    ...
    $sql = 'INSERT INTO `task`(`name`, `employee`, `deadline`, `deadtime`, `describ`, `file`, `process`, `feedback`, `file_submit`, `department`,`date`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
    $date = date("Y/m/d h:i");
    $stm = $dbCon->prepare($sql);
    $stm->bind_param("sssssssssss", $name, $employee, $deadline, $deadtime, $describ, $file_output, $process, $none, $none, $department,$date);
    $stm->execute();
